...
yvals = [1., 4.]
# second argument to root function is ndf -> 1 here vs 2 in 2D scan? Seems to work?
yvals = [ROOT.Math.chisquared_quantile_c(1 - 0.68, 1), ROOT.Math.chisquared_quantile_c(1 - 0.95, 1)]
...
textfit = '%s = %.3f{}^{#plus %.3f}_{#minus %.3f}' % (fixed_name, val_2sig[0], val_2sig[1], abs(val_2sig[2]))
...
if args.breakdown is None:
    for i, other in enumerate(other_scans):
        textfit = '#color[%s]{%s = %.3f{}^{#plus %.3f}_{#minus %.3f}}' % (other_scans_opts[i][2], fixed_name, other['val_2sig'][0], other['val_2sig'][1], abs(other['val_2sig'][2]))
        pt.AddText(textfit)
